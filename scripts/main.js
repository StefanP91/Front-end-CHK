...
window._show_pc_confirmation = function(id, header, detail, show, email) {
    ...
    hideButton.addEventListener('click', function() {
        var submitButton = document.querySelector('#_form_827_submit');
        submitButton.disabled = false;
        submitButton.classList.remove('processing');
        var mp = document.querySelector('input[name="mp"]');
        mp.value = '1';
    ...
    script.onerror = function() {
        if (isSubmit) {
            if (script.src.length > 10000) {
                _show_error("827", "Sorry, your submission failed. Please shorten your responses and try again.");
            } else {
                _show_error("827", "Sorry, your submission failed. Please try again.");
            }
            submitButton.disabled = false;
            submitButton.classList.remove('processing');
        }
...
(function() {
    ...
    var _removed = false;
        var form_to_submit = document.getElementById('_form_827_');
    ...
    const formSupportsPost = false;
        ...
        if (validate_form()) {
            ...
            var submitButton = e.target.querySelector('#_form_827_submit');
            submitButton.disabled = true;
            submitButton.classList.add('processing');
                        var serialized = _form_serialize(
                document.getElementById('_form_827_')
            ).replace(/%0A/g, '\\n');
